...
Framebuffer::Framebuffer(const nanogui::Vector2i &size, int numColorAttachments,
                         bool useDepthAttachment) : mDepth(useDepthAttachment ? new Texture2D(size, GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT) : nullptr)
{
...
Framebuffer::Framebuffer(std::vector<Texture2D> colorAttachments,
                         std::unique_ptr<Texture2D> depthAttachment) : mColor(std::move(colorAttachments)),
