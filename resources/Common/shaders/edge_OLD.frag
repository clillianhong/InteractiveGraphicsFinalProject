...
float fractionOccluded(mat4 frameToEye, int numSamples, vec3 eyePos, float occlusionRange) {
    ...
    for (int x = 0; x < numSamples; x++) {
        ...
        // Skip if point is too close (used to be if too far with a greater-than operator)
        ...
        if (ptDepth > actualDepth + 0.00001
            // texture(shaderParamTexture, ptInScreen.xy).z < 0.5
            // texture(shaderParamTexture, geom_texCoord).z < 0.5
            /*&& ptDepth - actualDepth < 0.005 * occlusionRange*/) {
            numOccluded++; 
        }
    }

    // return float(numOccluded > 0);
...
void main() {
    ...
    // } else {
        ...
        fragColor = (fracOcc > 0.01) ? vec4(0, 0, 0, 1) : texture(image, geom_texCoord);
